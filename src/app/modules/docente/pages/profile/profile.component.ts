...
,private docenteService: DocenteService,
  ...
  ngOnInit(): void {
    ...
    if (this.docenteDataService.docenteData && this.docenteDataService.docenteData.id) {
    ...
    this.especialidadesService.obtenerEspecialidadesPorDocente(docenteId).subscribe({
      next: (response) => {
        ...
        if (response.especialidades && response.especialidades.length > 0) {
          ...
          this.selectedEspecialidades_doce = this.especialidades.map(esp => esp.id);
          if (this.selectedEspecialidades_doce.length > 0) {
            ...
            this.mensajeEspecialidades = "No hay especialidades seleccionadas por el momento.";
...
//         docenteDataToSave.especialidades = this.selectedEspecialidades; // Siempre actualizar especialidades
//     }
...
//                 tap((response) => {
//                     docenteDataToSave.foto_url = response.image; // Solo se añade si se sube una nueva imagen
//                 })
//             )
//         );
//     }
...
//                 tap((response) => {
//                     docenteDataToSave.curriculum_file_url = response.fileUrl; // Solo se añade si se sube un nuevo archivo
//                 })
//             )
//         );
//     }
...
//         uploads.push(
//             this.fileUploadService.uploadDocumentoIdentificacion(documentoIdentificacionFile).pipe(
//                 tap((response) => {
//                     docenteDataToSave.documento_identificacion_file_url = response.fileUrl; // Solo se añade si se sube un nuevo archivo
//                 })
//             )
//         );
//     }
...
//                 tap((response) => {
//                     docenteDataToSave.cedula_file_url = response.fileUrl; // Solo se añade si se sube un nuevo archivo
//                 })
//             )
//         );
//     }
...
//             // Solo enviar datos si hay algo que actualizar
//             const updatedData = {
//                 ...currentDocenteData, // Mantener los datos existentes
//                 ...docenteDataToSave // Solo añadir cambios
//             };

//             // Eliminar propiedades con valor null o undefined antes de enviar
...
//             });
//             if (this.selectedEspecialidades.length > 0 &&
//               this.selectedEspecialidades.toString() !== (this.docenteData.especialidades || []).toString()
//             ) {
...
//             if (Object.keys(docenteDataToSave).length > 0) {
//                 this.docenteService.updateDocente(this.docenteData.id, updatedData).subscribe({
//                     next: (response) => {
...
//                 });
//             } else {
//                 console.log('No hay cambios para guardar, se conservan los datos existentes.');
//             }
//         },
//         error: (err) => {
...
guardarCambios() {
  ...
  if (documentoIdentificacionFile) {
    uploads.push(
      this.fileUploadService.uploadDocumentoIdentificacion(documentoIdentificacionFile).pipe(
        tap((response) => {
          docenteDataToSave.documento_identificacion_file_url = response.fileUrl; // Se añade si se sube un nuevo archivo
        })
      )
    );
  }
...
limpiarDatos(data: Partial<Docente>) {
    Object.keys(data).forEach((key) => {
        ...
        if (!data[typedKey] && data[typedKey] !== 0) { // Acepta 0 como un valor válido
