...
export class RegistroUserComponent implements OnInit {
  ...
  const selectedPlantel = this.planteles.find(plantel => plantel.id === Number(selectedPlantelId));
  ...
  if (selectedArea) {
    ...
    console.error('Área no encontrada para el ID seleccionado:', this.area_id);
  }
    ...
    this.especialidadesService.getEspecialidades().subscribe({
      next: (especialidad) => {
        this.especialidadesFiltradas = especialidad.filter((area: any) => area.area_id === this.area_id);
  ...
  onEspecialidadSelect(event: Event) {
    ...
    const selectedEspecialidad = this.especialidadesFiltradas.find(especialidad => especialidad.id === Number(selectedEspecialidadId));
      console.log(selectedEspecialidad)
    if (selectedEspecialidad) {
      console.log('Nombre del especialidad seleccionada:', selectedEspecialidad.nombre);
      this.formDataNAME.especialidad = selectedEspecialidad.nombre;
    } else {
      console.error('especialidad para el ID seleccionado:', selectedEspecialidadId);
    }
    ...
    this.cursosService.getCursos().subscribe({
      next: (cursos) => {
        ...
        this.cursosFiltrados = cursos.filter((curso: any) => curso.especialidad_id === Number(selectedEspecialidadId));
  ...
  onCursoSelect(event: Event) {
    ...
    const selectedCurso = this.cursosFiltrados.find(curso => curso.id === Number(selectedCursoId));
    //   console.log(selectedCurso)
    if (selectedCurso) {
      console.log('Nombre del especialidad seleccionada:', selectedCurso.nombre);
